...
// and egui falls back to `Theme::Dark`, so without this module every Linux user
// sees a dark window and no desktop setting reaches them.
//
// **The measurement is slipcase-desktop's, on 2026-08-28**, and this file is a
// copy of what it wrote. That repository has the run and the pixel samples in
// its `git log` and its `src/system_theme.rs` is the copy of record: with GNOME
// set to Light the window drew dark, and with `color-scheme` forced to
// `prefer-light` it drew dark again while the window's own titlebar turned
// light in the same screenshot. One window, two halves, disagreeing.
//
// Copied rather than shared, for now. It is four small platform facts and a
// D-Bus call with no product in it, which is the shape of a thing that belongs
// in a crate of its own — the fleet's rule is that an extraction happens once
// the duplicate has survived, and this is the second copy. `~/notes/dry_boundary.md`
// is the test; `src/opens_with.rs` in slipcase-desktop is the other candidate
// and the larger one.
//
...
/// still got the dark window, which is the defect this module was written for
...
mod tests {
    ...
    /// anything but light gives every GNOME user who chose Light the dark window
